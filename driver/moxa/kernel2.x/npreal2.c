...
static int npreal_block_til_ready(struct tty_struct *, struct file *,
		struct npreal_struct *);
static int npreal_startup(struct npreal_struct *, struct file *,
		struct tty_struct *);
static void npreal_shutdown(struct npreal_struct *);
static int npreal_port_shutdown(struct npreal_struct *);
static int npreal_get_serial_info(struct npreal_struct *,
		struct serial_struct *);
static int npreal_set_serial_info(struct npreal_struct *,
		struct serial_struct *);
...
static int npreal_set_modem_info(struct npreal_struct *, unsigned int,
		unsigned int *);
#endif
...
static int npreal_wait_command_completed(struct nd_struct *, char, char, long,
		char *, int *);
...
static int npreal_net_select(struct inode *, struct file *file, int,
		select_table *);
#endif
/*
 *  "proc" table manipulation functions
 */
static struct proc_dir_entry *npreal_create_proc_entry(const char *, mode_t,
...
#endif
	printk(
			"MOXA Async/NPort server family Real TTY driver ttymajor %d calloutmajor %d verbose %d (%s %s)\n",
			ttymajor, calloutmajor, verbose, NPREAL_VERSION, NPREAL_BUILD);
...
//	DRV_VAR_P(flush_buffer) = npreal_flush_buffer;
...
#endif
	DBGPRINT(MX_DEBUG_INFO,
			"Tty devices major number = %d, callout devices major number = %d\n",
			ttymajor, calloutmajor);
	...
	if (!(ret1 = tty_register_driver(DRV_VAR))) {
		...
		if ((ret2 = tty_register_driver(&npvar_cdriver))) {
			...
			DBGPRINT(MX_DEBUG_ERROR,
					"Couldn't install MOXA Async/NPort server family Real TTY callout driver !\n");
		}
#endif
	}
		...
		DBGPRINT(MX_DEBUG_ERROR,
				"Couldn't install MOXA Async/NPort server family driver !\n");
	}
...
#endif
		DBGPRINT(MX_DEBUG_ERROR,
				"Couldn't install MOXA Async/NPort server family Real TTY driver !\n");
	}
	return (0);
}
...
#if 0  // This code block will make driver ref count become negative when open port faile       
		info->count--;
		MX_MOD_DEC;
#endif        
		return (-EIO);
...
// Comment out the following two if's.
	...
	if ((tty->count == 1) && (info->count != 1)) {
		 ...
		 */
		DBGPRINT(MX_DEBUG_WARN,
				"[%d] npreal_close: bad serial port count; tty->count is 1, info->count is %d\n",
				current->pid, info->count);
		info->count = 1;
	}
	if (--info->count < 0) {
		DBGPRINT(MX_DEBUG_WARN,
				"npreal_close: bad serial port count for port %d: %d\n",
...
static void npreal_flush_buffer(struct tty_struct * tty) {
	...
	if (!(nd = info->net_node))
		return;
	nd->tx_ready = 0;
	if (npreal_wait_and_set_command(nd, NPREAL_ASPP_COMMAND_SET, ASPP_CMD_FLUSH)
			< 0)
		return;
	nd->cmd_buffer[2] = 1;
	...
	ASPP_CMD_FLUSH, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length);
}
...
// Scott	tty->driver_data = info;
	...
	if (!(nd->flag & NPREAL_NET_TTY_INUSED)) {
		...
		NPREAL_CMD_TIMEOUT, // Scott MAX_SCHEDULE_TIMEOUT,
				rsp_buffer, &rsp_length) != 0) {

			DBGPRINT(MX_DEBUG_ERROR,
					"wait for LOCAL_CMD_TTY_USED response failed\n");
			npreal_wait_and_set_command(nd, NPREAL_LOCAL_COMMAND_SET,
					LOCAL_CMD_TTY_UNUSED);
			nd->cmd_buffer[2] = 0;
			nd->cmd_ready = 1;
	...
	if (npreal_wait_command_completed(nd,
	NPREAL_ASPP_COMMAND_SET,
	ASPP_CMD_TX_FIFO, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length) != 0) {
...
//    nd->cmd_buffer[0] = NPREAL_LOCAL_COMMAND_SET;
	...
	if (npreal_wait_command_completed(nd,
	NPREAL_LOCAL_COMMAND_SET,
	LOCAL_CMD_TTY_UNUSED, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length) != 0) {
		npreal_wait_and_set_command(nd,
		NPREAL_LOCAL_COMMAND_SET,
		LOCAL_CMD_TTY_UNUSED);
		nd->cmd_buffer[2] = 0;
		...
		npreal_wait_command_completed(nd,
		NPREAL_LOCAL_COMMAND_SET,
		LOCAL_CMD_TTY_UNUSED, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length);
	}
	nd->flag &= ~NPREAL_NET_TTY_INUSED;
	return (0);
}
...
// since it will be used as a flag indicating closing wait none.
	...
	if (info->flags & ASYNC_INITIALIZED) {
		...
		if (info->net_node) {
			npreal_wait_and_set_command(info->net_node, NPREAL_ASPP_COMMAND_SET,
					ASPP_CMD_TX_FIFO);
...
static void npreal_start_break(struct nd_struct *nd) {
	...
	if (npreal_wait_command_completed(nd,
	NPREAL_ASPP_COMMAND_SET,
	ASPP_CMD_START_BREAK, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length))
		return;
	if (rsp_length != 4)
		return;
	if ((rsp_buffer[2] != 'O') || (rsp_buffer[3] != 'K'))
		return;
}
...
static void npreal_stop_break(struct nd_struct *nd) {
	...
	if (npreal_wait_command_completed(nd,
	NPREAL_ASPP_COMMAND_SET,
	ASPP_CMD_STOP_BREAK, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length))
		return;
	if (rsp_length != 4)
		return;
	if ((rsp_buffer[2] != 'O') || (rsp_buffer[3] != 'K'))
...
static int npreal_set_modem_info(struct npreal_struct * info, unsigned int cmd,
	...
	case TIOCMBIC:
		...
		break;
	case TIOCMSET:
		info->modem_control = ((info->modem_control
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 */
static struct proc_dir_entry *npreal_create_proc_entry(const char *name,
...
static void npreal_remove_proc_entry(struct proc_dir_entry *pde) {
	if (!pde)
		return;
...
static int npreal_net_select(struct inode *inode, struct file *file, int mode,
		select_table *table) {
 ...
 */
#if (LINUX_VERSION_CODE < VERSION_CODE(2,1,0))
static int npreal_net_read(struct inode *inode, struct file *file, char *buf,
		int count)
#else
static ssize_t
npreal_net_read (
...
#endif
{
	...
	while (count) {
		cnt = MIN(count,
				MIN( info->xmit_cnt, SERIAL_XMIT_SIZE - info->xmit_tail));
		if (cnt <= 0)
			break;
	...
	if (info->xmit_cnt <= 0) {
		nd->tx_ready = 0;
	} else { /* if not read all, wake up again to read.*/
		nd->tx_ready = 1;
		if (waitqueue_active(&nd->select_in_wait))
			wake_up_interruptible(&nd->select_in_wait);
	}
...
//	DBGPRINT(MX_DEBUG_TRACE, "flip.count=%d\n", tty->flip.count);
				 ...
				 * set ASPP command
				 */
static int npreal_wait_and_set_command(struct nd_struct *nd, char command_set,
		char command) {
	...
	while (1) {
		...
		if (nd->cmd_buffer[0] == 0) {
			...
			up(&nd->cmd_semaphore);
			return (0);
		} else if ((jiffies >= et) || signal_pending(current)) { // timeout
			nd->cmd_buffer[0] = command_set;
			nd->cmd_buffer[1] = command;
			up(&nd->cmd_semaphore);
			return (0);
 ...
 */
static int npreal_wait_command_completed(struct nd_struct *nd, char command_set,
		char command, long timeout, char *rsp_buf, int *rsp_len) {
	long et = 0;
...
static int npreal_linectrl(struct nd_struct *nd, int modem_control) {
	...
	if (npreal_wait_command_completed(nd,
	NPREAL_ASPP_COMMAND_SET,
	ASPP_CMD_LINECTRL, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length))
		return (-EIO);
	if (rsp_length != 4)
		return (-EIO);
	if ((rsp_buffer[2] != 'O') || (rsp_buffer[3] != 'K'))
...
static int npreal_setxon_xoff(struct npreal_struct * info, int cmd) {
	...
	if (npreal_wait_command_completed(nd,
	NPREAL_ASPP_COMMAND_SET, cmd, NPREAL_CMD_TIMEOUT, rsp_buffer, &rsp_length))
		return (-EIO);
	if (rsp_length != 4)
		return (-EIO);
	if ((rsp_buffer[2] != 'O') || (rsp_buffer[3] != 'K'))
